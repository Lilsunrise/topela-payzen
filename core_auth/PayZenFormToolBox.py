...
class PayZenFormToolBox:
    ...
    def ipn(self, request):
        ...
        Keyword arguments:
        request -- HttpRequest object, the request received from PayZen platform

        Returns:
        a valid Django HttpResponse (status string)

        Throws:
        Exception if the data signature is incorrect
        Exception if the action is not 'PAY', 'BATCH_AUTO' or 'BO'
        """
        self.logger.debug("IPN request with fields: " + json.dumps(request.POST))
        data = {}
        ...
        if signature != request.POST.get('signature'):
            err = 'Signature mismatch - Payment is not confirmed (received: {} / computed: {})'.format(
                request.POST.get('signature'), signature)
            self.logger.warning(err)
            return HttpResponse(err, status=400)

        if request.POST.get('vads_url_check_src') in ['PAY', 'BATCH_AUTO']:
            return self.ipn_pay(data)

        if request.POST.get('vads_url_check_src') == 'BO':
            return HttpResponse('Hello PayZen BO!')

        return HttpResponse("IPN action unhandled: " + request.POST.get('vads_url_check_src'), status=400)
    ...
    def ipn_pay(self, fields):
        ...
        if fields['vads_trans_status'] in ['AUTHORISED_TO_VALIDATE',
                                            ...
                                            'UNDER_VERIFICATION']:
            raise Exception("Payment is not yet authorised - Given status is " + fields['vads_trans_status'])
